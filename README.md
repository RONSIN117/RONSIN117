<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=wave&color=8a2be2&height=200&section=header&text=Ronit%20Sinha&fontSize=50&animation=fadeIn&fontAlignY=35&desc=Software%20Engineering%20%7C%20Cybersecurity%20%7C%20AI/ML&descAlignY=55&descAlign=50&fontColor=ffffff" width="100%" />
</div>

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=22&pause=1000&color=9370DB&center=true&vCenter=true&width=800&height=60&lines=Second-Year+CSE+Undergraduate;Certified+Cybersecurity+Specialist;AI%2FML+%26+System+Hardening;Full+Stack+Development+%26+Automation" alt="Typing SVG" />
</div>

<div align="center">
  <img src="https://img.shields.io/badge/Saveetha%20Engineering%20College-0D1117?style=flat-square&logo=bookstack&logoColor=9370DB" alt="Academic" />
  <img src="https://img.shields.io/badge/Chennai,%20India-0D1117?style=flat-square&logo=googlemaps&logoColor=9370DB" alt="Location" />
</div>

<br>

<div align="center">
  <a href="mailto:ronitsinha188@gmail.com"><img src="https://img.shields.io/badge/Email-0D1117?style=flat-square&logo=gmail&logoColor=9370DB" alt="Email" /></a>
  <a href="https://www.linkedin.com/in/ronit-sinha-0a923136a/"><img src="https://img.shields.io/badge/LinkedIn-0D1117?style=flat-square&logo=linkedin&logoColor=9370DB&cache=none" alt="LinkedIn" /></a>
  <a href="https://github.com/RONSIN117"><img src="https://img.shields.io/badge/GitHub-0D1117?style=flat-square&logo=github&logoColor=9370DB" alt="GitHub" /></a>
  <a href="#"><img src="https://img.shields.io/badge/Portfolio-0D1117?style=flat-square&logo=vercel&logoColor=9370DB" alt="Portfolio" /></a>
</div>

<br>

<div align="center">
  <a href="https://github.com/RONSIN117"><img src="https://komarev.com/ghpvc/?username=RONSIN117&label=Profile%20Views&color=8a2be2&style=flat-square" alt="Profile Views" /></a>
  <a href="https://github.com/RONSIN117?tab=followers"><img src="https://img.shields.io/github/followers/RONSIN117?label=Followers&style=flat-square&color=8a2be2" alt="Followers" /></a>
  <a href="https://github.com/RONSIN117?tab=repositories"><img src="https://img.shields.io/github/stars/RONSIN117?label=Stars&style=flat-square&color=8a2be2" alt="Stars" /></a>
</div>

---

<div align="center">
  <h2>✦ About ✦</h2>
</div>

I am a second-year Computer Science and Engineering undergraduate specializing in Cybersecurity at Saveetha Engineering College, driven by a passion for building secure, resilient digital ecosystems. My technical foundation spans low-level system hardening, automated OSINT reconnaissance, and high-performance programming in C, C++, and Python. Certified across multiple core security and software disciplines—including credentials from Google, NIIT, and Infosys Springboard—I actively merge theoretical defensive paradigms with practical application engineering. Whether architecting autonomous sensor pipelines or investigating system vulnerabilities, I am dedicated to engineering secure-by-design solutions for complex technological challenges.

**Open To:** Software Engineering Internships • AI/ML Research • Cybersecurity Analyst Roles

---

<div align="center">
  <h2>✦ Tech Stack ✦</h2>
</div>

<div align="center">
  <h3>Languages</h3>
  <img src="https://skillicons.dev/icons?i=c,cpp,python,java&theme=dark" alt="Languages" />
  
  <h3>Frontend & Backend</h3>
  <img src="https://skillicons.dev/icons?i=html,css,js,nodejs,express&theme=dark" alt="Frontend and Backend" />

  <h3>Cloud, DevOps & Tooling</h3>
  <img src="https://skillicons.dev/icons?i=linux,kali,bash,git,github,vim&theme=dark" alt="DevOps and Tooling" />
</div>

---

<div align="center">
  <h2>✦ AI/ML Expertise ✦</h2>
</div>

| Domain | Proficiency | Details |
| :--- | :--- | :--- |
| **Generative AI Security** | Advanced | Integration of GenAI models within secure frameworks, threat modeling for AI pipelines. |
| **Data Analytics & Parsing** | Intermediate | High-volume telemetry data extraction, timing parsing, and algorithmic optimization. |
| **Automated Reconnaissance** | Intermediate | Python-driven OSINT, metadata leakage analysis, and predictive footprinting. |

---

<div align="center">
  <h2>✦ Featured Projects ✦</h2>
</div>

<details>
  <summary><b>❖ Caterpillar Tech Challenge: Scraper Gate</b></summary>
  <br>
  Designed a stationary autonomous recovery system utilizing advanced sensor arrays to remove residue from mining trucks.

  | Metric | Details |
  | :--- | :--- |
  | **Stack** | C++, LiDAR, ToF Sensors, Hardware Automation |
  | **Scale** | Enterprise Mining Infrastructure Prototype |
  | **Performance** | Optimized real-time sensor processing loop |
  | **Security** | Hardened sensor data transmission protocols |
  | **Impact** | Engineered a blueprint for autonomous operational efficiency |
  | **Repository** | [View Source Code](#) |

  *Professional Context:* Architected the logic and Bill of Materials for a highly specialized hardware-software bridge, converting raw ToF/LiDAR spatial data into actionable physical clearance events.
</details>

<details>
  <summary><b>❖ Social Media Reconnaissance Engine</b></summary>
  <br>
  An advanced footprinting and security assessment tool designed to evaluate metadata handling and data leakage on social platforms.

  | Metric | Details |
  | :--- | :--- |
  | **Stack** | Python, Instaloader, Kali Linux, Bash |
  | **Scale** | Targeted OSINT Pipeline |
  | **Performance** | Asynchronous data scraping and metadata parsing |
  | **Security** | Secure execution environment with proxy routing capabilities |
  | **Impact** | Identified critical metadata exposure vectors in standard user profiles |
  | **Repository** | [View Source Code](#) |

  *Professional Context:* Built an end-to-end reconnaissance pipeline for Instagram using Python and Kali Linux to automate the detection of structural vulnerabilities and assess digital footprints for compliance and security hardening.
</details>

<details>
  <summary><b>❖ F1 Telemetry & Timing Analyzer</b></summary>
  <br>
  A high-performance data processing tool that ingests and analyzes live racing telemetry.

  | Metric | Details |
  | :--- | :--- |
  | **Stack** | Python, FastF1, Data Structures, Pandas |
  | **Scale** | Millions of data points per race session |
  | **Performance** | Optimized parsing of multi-channel API streams |
  | **Security** | Data validation and rate-limit handling |
  | **Impact** | Enabled deep-dive technical race replays and strategy forecasting |
  | **Repository** | [View Source Code](#) |

  *Professional Context:* Leveraged the FastF1 library to build a comprehensive data pipeline capable of visualizing complex race metrics, showcasing strong capabilities in algorithmic data parsing and application-level analytics.
</details>

---

<div align="center">
  <h2>✦ Experience ✦</h2>
</div>

**System Hardening & Penetration Testing Project Lead** | Independent Research
*February 2026 – Present*
* Engineered comprehensive vulnerability assessments and metadata leakage footprinting pipelines.
* Leveraged Wireshark for network traffic analysis and Nmap for advanced reconnaissance.
* Formulated strict compliance scripts including enforcing rigid environment variables (`pen=$5`) for testing automation.
* `Kali Linux` `Python` `Bash` `Network Security` `OSINT`

---

<div align="center">
  <h2>✦ Achievements ✦</h2>
</div>

| Recognition | Details |
| :--- | :--- |
| **Certified Cybersecurity Specialist** | Completed the Advanced Cybersecurity with GenAI Program, demonstrating mastery in modern threat vectors. |
| **Google Cybersecurity Milestones** | Successfully certified in both *Foundations of Cybersecurity* and *Play It Safe: Manage Security Risks* authorized by Google. |
| **Software Engineering Credential** | Completed comprehensive software engineering training and certification via Infosys Springboard. |
| **Competitive Programming** | Consistent active implementation of complex data structures (Linked Lists) and IPC mechanisms in C/C++. |

---

<div align="center">
  <h2>✦ Certifications ✦</h2>
</div>

<div align="center">
  <a href="https://coursera.org/verify/APC0OWOJN8LC" target="_blank"><img src="https://img.shields.io/badge/Google-Play%20It%20Safe%3A%20Manage%20Security%20Risks-8a2be2?style=flat-square&logo=google&logoColor=white" alt="Play It Safe" /></a>
  <a href="https://coursera.org/verify/DJ4FZZZFE8QN" target="_blank"><img src="https://img.shields.io/badge/Google-Foundations%20of%20Cybersecurity-8a2be2?style=flat-square&logo=google&logoColor=white" alt="Foundations of Cybersecurity" /></a>
  <a href="https://verify.onwingspan.com" target="_blank"><img src="https://img.shields.io/badge/Infosys%20Springboard-Software%20Engineering-8a2be2?style=flat-square&logo=infosys&logoColor=white" alt="Infosys Software Engineering" /></a>
  <br><br>
  <img src="https://img.shields.io/badge/NIIT-Cybersecurity_with_GenAI_Advanced_Program-8a2be2?style=flat-square&logo=security&logoColor=white" alt="NIIT CyberSec" />
</div>

---

<div align="center">
  <h2>✦ GitHub Analytics ✦</h2>
</div>

<div align="center">
  <img src="https://img.shields.io/badge/Primary_Language-C++_%2F_Python-0D1117?style=flat-square&logo=cplusplus&logoColor=9370DB" alt="Primary Language" />
  <img src="https://img.shields.io/badge/Focus-Cybersecurity_%26_Systems-0D1117?style=flat-square&logo=kalilinux&logoColor=9370DB" alt="Focus" />
</div>
<br>
<div align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=RONSIN117&theme=dracula&ring=8A2BE2&fire=9370DB&currStreakLabel=9370DB&background=0D1117&hide_border=true" alt="GitHub Streak" width="100%" />
</div>

---

<div align="center">
  <h2>✦ GitHub Trophies ✦</h2>
</div>

<div align="center">
  <img src="https://img.shields.io/badge/🏆_Milestone-Active_Contributor-0D1117?style=flat-square&logo=github&logoColor=9370DB" alt="Milestone" />
  <img src="https://img.shields.io/badge/⭐_Status-Open_Source_Builder-0D1117?style=flat-square&logo=git&logoColor=9370DB" alt="Status" />
</div>

---

<div align="center">
  <h2>✦ Contribution Activity ✦</h2>
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=RONSIN117&bg_color=0D1117&color=9370DB&line=8A2BE2&point=FFFFFF&area=true&hide_border=true" alt="Contribution Graph" width="100%" />
</div>

---

<div align="center">
  <h2>✦ Contribution Snake ✦</h2>
</div>

<div align="center">
  <img src="https://raw.githubusercontent.com/RONSIN117/RONSIN117/output/github-contribution-grid-snake-dark.svg" alt="Snake Animation" width="100%" />
</div>

---

<div align="center">
  <h2>✦ Current Focus ✦</h2>
</div>

```yaml
profile:
  learning:
    - "Google Cybersecurity"
  building:
    - "Pocket Auditor project"
  exploring:
    - "FastF1 data pipeline optimizations"
    - "Low-level Linux kernel vulnerabilities"
  open_to:
    - "Open Source Collaborations"
    - "Enterprise Security Internships"
